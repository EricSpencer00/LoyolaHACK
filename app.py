...
def load_routes():
    routes = []
    with open('google_transit/routes.txt', mode='r') as file:
...
# Realtime CTA Data for Bus Predictions
def get_cta_bus_data():
    ...
    if "bustime-response" in data and "prd" in data["bustime-response"]:
        for prd in data["bustime-response"]["prd"]:
            predictions.append({
                "lat": 41.880,  # Dummy coordinate; replace with actual stop coordinates if available
                "lng": -87.630,
                "line": prd.get("rt"),
...
# Realtime CTA Data for Train Predictions (Simulated)
def get_cta_train_data():
    ...
    if "traintracker-response" in data and "prd" in data["traintracker-response"]:
        for prd in data["traintracker-response"]["prd"]:
            predictions.append({
                "lat": 41.880,  # Dummy coordinate; replace with actual station coordinates
                "lng": -87.630,
                "line": prd.get("rt"),
                "arrival": prd.get("prdctdn")
            })
    else:
        # For demonstration, simulate train data
        predictions = [
...
def add_favorite():
    ...
    # Fetch the route by ID; in a real application, query your database
    route = next((r for r in routes if r["id"] == route_id), None)
    if not route:
        return jsonify({"status": "error", "message": "Route not found."}), 404

    favorites = user.get_favorites()
    if route["name"] in favorites:
        return jsonify({"status": "error", "message": "Route already in favorites."}), 400

    favorites.append(route["name"])
